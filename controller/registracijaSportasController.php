...
class registracijaSportasController extends BaseController
{
	public function index() 
	{
        ...
        {
            $this->registry->template->title = 'Unesite korisni�ko ime i lozinku';
        ...
        {
            $this->registry->template->title = 'Korisni�ko ime mora imati izme�u 3 i 10 znakova.';
            $this->registry->template->kluboviList = $rs->generirajKlubove();
			$this->registry->template->show( 'registracijaSportas_index' );
        }
        ...
        else
        {
            ...
            if( $user !== null )
            {
                $this->registry->template->title = 'Taj korisnik ve� postoji';
                $this->registry->template->kluboviList = $rs->generirajKlubove();
                $this->registry->template->show( 'registracijaSportas_index' );
            }
            else
